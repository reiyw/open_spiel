...
void TieBreak() {
  ...
  SPIEL_CHECK_EQ(state->LegalActions(), std::vector<int>({1, 2, 3, 4, 5}));
  state->ApplyAction(5);
  SPIEL_CHECK_EQ(state->LegalActions(), std::vector<int>({1, 2, 3, 4, 5}));
  state->ApplyAction(2);
  SPIEL_CHECK_EQ(state->LegalActions(), std::vector<int>({1, 2, 3, 4, 5}));
  state->ApplyAction(4);
  SPIEL_CHECK_EQ(state->LegalActions(), std::vector<int>({0, 1, 2, 3, 4}));
  state->ApplyAction(2);
  SPIEL_CHECK_EQ(state->LegalActions(), std::vector<int>({0, 1}));
  state->ApplyAction(1);
  SPIEL_CHECK_EQ(state->LegalActions(), std::vector<int>({0, 1, 2, 3}));
  state->ApplyAction(2);
  SPIEL_CHECK_EQ(state->LegalActions(), std::vector<int>({0, 2}));
  state->ApplyAction(2);
